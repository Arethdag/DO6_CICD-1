...
void charT(const char *table[static TABLES]) {
  const char *array[] = {
      ...
      "\xfc", "\xfd", "\xfe", "\xff"};
  memcpy(table, array, sizeof(array));
}

void flag_e(const char *table[static TABLES]) { table['\n'] = "$\n"; }

void flag_t(const char *table[static TABLES]) { table['\t'] = "^I"; }

void Cat(int argc, char *argv[], Flags flags,
         const char *table[static TABLES]) {
...
void flag_v(const char *table[static TABLES]) {
  const char *part1[] = {"^@", "^A", "^B", "^C", "^D", "^E", "^F", "^G", "^H"};

  const char *part2[] = {"^K", "^L", "^M", "^N",  "^O", "^P", "^Q",
                         "^R", "^S", "^T", "^U",  "^V", "^W", "^X",
                         "^Y", "^Z", "^[", "^\\", "^]", "^^", "^_"};
  ...
  const char *part3[] = {
      ...
      "M-}",  "M-~",  "M-^?"};
  memcpy(table, part1, sizeof part1);
  memcpy(&table[9 + 2], part2, sizeof part2);
  memcpy(&table['~' + 1], part3, sizeof part3);
}
